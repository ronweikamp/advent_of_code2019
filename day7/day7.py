...
def run_phase_setting(code, phase_settings):
    ...
    for p in phase_settings:
        _, input_output, _, _ = run(0, code, inputs=[p, input_output])
...
def run_feedbackloop_setting(code, phase_settings):
    ...
    for i in range(5):
        ...
        amps[i], input_output, indices[i], reason = run(indices[i], amps[i], inputs=[phase_settings[i]], halt_at_out=True)
    ...
    while not_halted:
        for i in range(5):
            amps[i], input_output, indices[i], reason = run(indices[i], amps[i], inputs=[input_output], halt_at_out=True)
            print('out {} reason {}'.format(input_output, reason))
